...
def optimize_acqf_and_get_observation(model, heldout_inputs, heldout_outputs):
    ...
    acq_vals = UpCoBo(model, heldout_inputs, beta=0.1).detach()
...
def BO_framework_graph(X_set, y_set, acquisition_function, 
                       start_set_size = 0.1, bo_n_iteration = 20,
                       reproducibility = False, trial = 0,
                       gp_loop_fit = False) -> Tuple[torch.Tensor, torch.Tensor]:
    '''
    ...
    #Since RND and BO will be compared each of them will have a candidate pool test_y, (Test_x is irrelevant for the RND as choises are made by chance)
    test_y_rnd = test_y
    ...
    for iteration in range(bo_n_iteration):
        ...
        #Get general information from all the statistical GP model:
        ...
        new_x_BO, new_y_BO, test_x, test_y, acq_vals = optimize_acqf_and_get_observation(gp_model,test_x,test_y)
        train_x = NonTensorialInputs(train_x.data + [new_x_BO])
        ...
        selected_sample = gp_model(NonTensorialInputs([new_x_BO]))
